...
<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
      ...
      >
        Quản Lý Lịch Sử Giao Dịch (Admin)
      </h1>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div class="flex-1 min-w-48">
            ...
            <Dropdown
              ...
              class="w-full"
              inputClass="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-gray-700 dark:text-gray-300"
            />
          </div>
    ...
    <!-- Transaction Table -->
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6">
      <TransactionTable :transactions="paginatedTransactions" :isAdmin="true" />
    </div>

    <!-- Pagination -->
    <Pagination
      v-if="adminTopupHistory?.pagination"
      :current-page="adminTopupHistory.pagination.page"
      :total-pages="adminTopupHistory.pagination.totalPages"
      :total-items="adminTopupHistory.pagination.total"
      :limit="adminTopupHistory.pagination.limit"
...
const adminTopupHistory = computed(() => billingStore.adminTopupHistory)
const topups = computed(() => billingStore.adminTopupHistory?.topups || [])
...
const appliedFilters = ref<AdminTopupQuery>({
  page: 1,
  limit: 10,
  status: '',
  paymentMethod: '',
...
const totalTransactions = computed(() => adminTopupHistory.value?.pagination?.total || 0)

const totalAmount = computed(() => {
  return topups.value
    .filter(t => t.paymentMethod !== 'PAYMENT' && t.status === 'completed') // Only count completed topups
...
const handleSearch = async () => {
  ...
  appliedFilters.value = {
    ...
    startDate: searchForm.value.startDate instanceof Date ? searchForm.value.startDate.toISOString() : undefined,
    endDate: searchForm.value.endDate instanceof Date ? searchForm.value.endDate.toISOString() : undefined,
  }
  // Call API to get filtered results
  await billingStore.getAllTopupHistoryByAdmin(appliedFilters.value)
...
const handlePageChange = async (page: number) => {
  ...
  const query: AdminTopupQuery = {
    ...searchForm.value,
    startDate: searchForm.value.startDate instanceof Date ? searchForm.value.startDate.toISOString() : undefined,
    endDate: searchForm.value.endDate instanceof Date ? searchForm.value.endDate.toISOString() : undefined,
  }
  await billingStore.getAllTopupHistoryByAdmin(query)
}
...
onMounted(async () => {
  ...
  await billingStore.getAllTopupHistoryByAdmin(query)
  await userStore.getUsers() // Fetch all users on mount
})
</script>
