    ...
    <div class="bg-white rounded-xl shadow-2xl max-w-md w-full p-6">
      <div class="flex justify-between items-center mb-4">
        <h3 class="text-xl font-bold text-gray-900">
          {{ props.isTarget ? 'Thêm Link Đích Mới' : 'Thêm Link Video Or Image Preview Mới' }}
        </h3>
      ...
      <div class="space-y-4">
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">
            {{
              props.isTarget ? 'Link đích được chuyển tới' : 'Link Preview (YouTube, Image, v.v.)'
            }}
          </label>
          <input
            v-model="link"
            type="url"
            :placeholder="
              props.isTarget ? 'https://drive.google.com/...' : 'https://example.com/...'
            "
...
const link = ref('')
const type = ref<LINK_TYPES>(
  props.isTarget ? LINK_TYPES.DESTINATION_URL : LINK_TYPES.INTERSTITIAL_IMAGE,
)
